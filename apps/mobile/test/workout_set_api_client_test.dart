...
void main() {
  group('WorkoutSetApiClient', () {
    test('create, update, assign, and unassign use expected payloads and paths', () async {
      final seen = <String>[];
      final client = WorkoutSetApiClient(
        ...
        httpClient: MockClient((request) async {
          ...
          if (request.url.path == '/workout-sets/set-1' && request.method == 'PUT') {
            ...
            return http.Response(jsonEncode(_detailJson(name: 'Push B')), 200);
          }
      ...
      await client.create(accessToken: 'access-token', request: _createRequest('Push A'));
      await client.update(
        accessToken: 'access-token',
        workoutSetId: 'set-1',
        request: UpdateWorkoutSetRequest(name: 'Push B', rows: _rowsRequest()),
      );
      await client.assign(
        accessToken: 'access-token',
        workoutSetId: 'set-1',
    ...
    test('list and detail methods parse trainer and trainee responses', () async {
      final seen = <String>[];
      final client = WorkoutSetApiClient(
        baseUrl: 'https://api.example.test',
        httpClient: MockClient((request) async {
      ...
      final summaries = await client.listTrainerSets(accessToken: 'access-token');
      final detail = await client.getTrainerSet(
        accessToken: 'access-token',
        workoutSetId: 'set-1',
      );
      final traineeList = await client.listTraineeSets(accessToken: 'access-token');
      ...
      expect(detail.data?.rows.single.exerciseType, ExerciseValueType.repsWeight);
    ...
    test('maps forbidden, conflict, missing base URL, and invalid JSON', () async {
      ...
      expect(invalidJsonResult.message, contains('Invalid workout set detail'));
    });
  });
}
