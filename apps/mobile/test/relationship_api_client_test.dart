...
void main() {
  group('RelationshipApiClient', () {
    test(
      'trainer relationship sends bearer token and parses trainees',
      () async {
        final client = RelationshipApiClient(
          ...
          httpClient: MockClient((request) async {
            ...
            expect(
              ...
              'https://api.example.test/trainer/relationship',
            );
    ...
    test(
      'maps authorization failures without parsing body as success',
      () async {
    ...
    test('returns error for invalid relationship JSON', () async {
      ...
      expect(
        result.message,
        contains('Invalid trainer relationship response JSON'),
      );
    });

    test('formats weekly streak and relative workout copy in Polish', () {
      expect(formatWeekCount(0), '0 tygodni');
      expect(formatWeekCount(1), '1 tydzień');
      expect(formatWeekCount(2), '2 tygodnie');
      expect(formatWeekCount(5), '5 tygodni');
      expect(formatWeekCount(12), '12 tygodni');
      expect(formatWeeklyStreakFlame(6), '🔥 6');

      final now = DateTime(2026, 6, 24, 12);
      expect(formatLastWorkout(null, now: now), 'nie zaczął');
      expect(formatLastWorkout(DateTime(2026, 6, 24, 8), now: now), 'dzisiaj');
      expect(formatLastWorkout(DateTime(2026, 6, 23, 18), now: now), 'wczoraj');
      expect(
        formatLastWorkout(DateTime(2026, 6, 20, 18), now: now),
        '4 dni temu',
      );
    });
  });
