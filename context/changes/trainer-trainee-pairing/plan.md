...
- [x] 1.4 A trainer can call `GET /trainer/relationship` and receive an invite code plus only their linked trainees. — 7f9c537
- [x] 1.5 A trainee can call `GET /trainee/relationship` before and after pairing and see `null` then trainer identity. — 7f9c537
- [x] 1.6 A trainee linked to trainer A can submit trainer B's valid code and `/auth/me` then shows trainer B. — 7f9c537
- [x] 1.7 After re-pair, trainer A can no longer access the previous shared session for that trainee, and that session is `cancelled`. — 7f9c537
...
- [x] 2.5 Mobile calls `GET /trainer/relationship` with a trainer token after login. — 7f9c537
- [x] 2.6 Mobile calls `GET /trainee/relationship` with a trainee token after login. — 7f9c537
- [x] 2.7 Mobile re-pairing uses `AuthController.claimTrainerInviteCode`; relationship state reloads after success. — 7f9c537
...
- [x] 3.4 Trainer login lands on a dark `t_dash`-style screen, not the old diagnostic/temporary panel. — 7f9c537
- [x] 3.5 Trainer with no trainees sees the invite-code card and `Zaproś podopiecznego` CTA. — 7f9c537
- [x] 3.6 Trainer with at least one trainee sees that trainee in the list and can open detail. — 7f9c537
- [x] 3.7 Trainee login with a trainer lands on a `c_home`-style screen showing trainer identity. — 7f9c537
- [x] 3.8 Trainee login without a trainer sees a clear code-entry prompt. — 7f9c537
- [x] 3.9 Trainee can enter a second trainer's valid code and the displayed trainer changes. — 7f9c537
- [x] 3.10 Text and controls fit on a phone-sized viewport without overlap. — 7f9c537
